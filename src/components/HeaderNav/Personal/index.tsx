...
export const Personal: React.VFC<{className?: string}> = (props) => {
  ...
  if (!state.authenticated) return <NeedLogin {...props} callLogin={login} />;
  else if (state.viewer === undefined) return <Fetching {...props} />;
  else if (state.viewer === null)
    return <NeedToRegister {...props} callLogin={register} />;
  else return <Registered {...props} viewer={state.viewer} />;
};
